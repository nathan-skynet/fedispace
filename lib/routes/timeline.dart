...
class Timeline extends StatefulWidget implements PreferredSizeWidget{
  final ApiService apiService ; String typeTimeLine;

  /// Main instance of the API service to use in the widget.
   Timeline({Key? key, required this.apiService, required this.typeTimeLine }) : super(key: key);
  @override
...
class _TimelineTabsState extends State<Timeline> with TickerProviderStateMixin{
  Account? account;  static const _pageSize = 20;
  ...
  Future<void> _fetchPage(String? lastStatusId, String typeTimeLine) async {
    try {
      final List<Status> newStatuses = await widget.apiService.getStatusList(lastStatusId, _pageSize,typeTimeLine);
